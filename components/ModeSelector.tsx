...
const ModeSelector: React.FC<ModeSelectorProps> = ({ currentMode, onModeChange }) => {
  const modes = [
    ...
    { id: TorchMode.MORSE, label: 'Morse', icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z' },
    ...
    { id: TorchMode.MOOD, label: 'Mood', icon: 'M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z' },
  ];

  return (
    <div className="flex overflow-x-auto gap-2 p-2 no-scrollbar glass rounded-3xl items-center justify-start">
      {modes.map((mode) => (
        <button
          key={mode.id}
          onClick={() => onModeChange(mode.id)}
          className={`flex flex-col items-center justify-center min-w-[70px] py-2 px-1 rounded-2xl transition-all duration-300 ${
            currentMode === mode.id 
